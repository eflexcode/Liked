...
public class MessageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        messageText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.toString().isEmpty()) {
                    send.setImageResource(R.drawable.send_no);
                } else {
        ...
        viewModel.observeUserDetails().observe(this, new Observer<User>() {
            ...
            public void onChanged(User user) {
                ...
                Glide.with(MessageActivity.this).load(user.getProfilePictureUrl())
                        .apply(requestOptions).into(proPicChat);

                imageUrl = user.getProfilePictureUrl();

                try {
                    if (user.getIsOnline().equals("yes")) {
                        onLine.setImageResource(R.color.on_line);
                    } else {
                        onLine.setImageResource(R.color.off_line);
                    }
                }catch (NullPointerException e){
        ...
        viewModel.observeGetChats().observe(this, new Observer<List<Chat>>() {
            ...
            public void onChanged(List<Chat> chats) {
                chatAdapter = new ChatAdapter(chats,MessageActivity.this);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 9 && resultCode == RESULT_OK && data.getData() != null) {
            isCamera = false;
            viewModel.sendImageGallery(id, FirebaseAuth.getInstance().getUid(), data.getData());
        ...
        if (requestCode == 10 && resultCode == RESULT_OK) {
            ...
            viewModel.sendImageCamera(id, FirebaseAuth.getInstance().getUid(), bytes);
            Toast.makeText(this, "sending", Toast.LENGTH_SHORT).show();
            isCamera = true;
